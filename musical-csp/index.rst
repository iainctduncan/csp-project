...
This also simplifies some of the implementation details for the search algorithm as the search
function can have direct access to local state that we use for variables and domains, while
still insulating these from client code.

A simple example of the object structure is shown below:
...
The solver is set up via an explicit initialization method which restores domains for variables
and resets the variable assignments to null, allowing the same solver to be reused for subsequent
bars by reinitializing and setting only the values that need to change for each bar (i.e. chord).
...
of caching, thus avoiding unnecessary computation during constraint execution, which
...
to the index of the note variable; and the value being tested as a MIDI note symbol. 
...
During the solve process, the solver uses the **get-applicable-constraints** method
...
(because running it at that point is meaningless and should not indicate a failure),
...
This was necessary for the test program as the chromatic neighbour
of a target root may or may not be already in the chord. We thus need a way to specify, 
*"all notes should be different unless the duplicated note is note 4 functioning as a 
chromatic neighbour to the following (target) note"*.
...
Our constraints are:
  ...
  * The second and third notes must be in the chord
  ...
  * The penultimate note is a chromatic neighbour of the final note
...
I may want it for some future features, such as domain change rewinding where we
may need to keep track of where we are in the tree of state-space traversed.
...
In the case that a solution is found, the enclosing method returns
the note assignments as a list, in addition to having the side effect of populating the local note assignment
list, this to simplify using the object.
...
Musical Assumptions
^^^^^^^^^^^^^^^^^^^^
The current implementation assumes that there is only one chord per bar and that
we have a set number of notes per bar that remains the same.
Rules for two-beat chords have not been implemented and the rules stay the same for each bar
in the test program.
...
This structure could be used to store domain changes from visiting and assigning a node, which could be used
...
^^^^^^^^^^^^^^^^^^^^^^
As discussed above, the lack of constraint propagation is the first major deficiency.
...
could be an issue, however, for solving longer lines, as with a range of five octaves,
the maxium state space will grow exponentially at powers of 60 with each note.
...
takes the next one from the list of domain values or a random value. The addition of preference constraints
